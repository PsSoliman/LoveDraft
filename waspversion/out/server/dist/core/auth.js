...
const auth = handleRejection(async (req, res, next) => {
    ...
    if (authHeader.startsWith('Bearer ')) {
        ...
        const { password } = user, userView = __rest(user, ["password"]);
        req.user = userView;
    }
    else {
        throwInvalidCredentialsError();
...
export function generateAvailableDictionaryUsername() {
    ...
    for (let i = 0; i < 10; i++) {
        const potentialUsername = `${adjectives[randomInt(adjectives.length)]}-${colors[randomInt(colors.length)]}-${nouns[randomInt(nouns.length)]}-${randomInt(100000)}`;
...
export function generateAvailableUsername(usernameSegments, config) {
    const separator = (config === null || config === void 0 ? void 0 : config.separator) || '-';
    const baseUsername = usernameSegments.join(separator);
    const potentialUsernames = [];
    for (let i = 0; i < 10; i++) {
        const potentialUsername = `${baseUsername}${separator}${randomInt(100000)}`;
        potentialUsernames.push(potentialUsername);
    }
    return findAvailableUsername(potentialUsernames);
}
